...
Normally, one would use this package in a sensitive-data environment, where one could e.g. read in the registry by running the below code:

```{r iiCBT_readin_example, eval=FALSE}
...
```{r iiCBT_build_example, warning=FALSE, message=FALSE}
...
```{r FPQR_build_example, warning=FALSE, message=FALSE}
...
We include functions from the paper:
Rosenström, T. H., Czajkowski, N. O., Solbakken, O. A., & Saarni, S. E. (2023). Direction of dependence analysis for pre-post assessments using non-Gaussian methods: A tutorial. Psychotherapy Research. [https://doi.org/10.1080/10503307.2023.2167526](https://doi.org/10.1080/10503307.2023.2167526)

For use instructions, see the paper or the [Bitbucket repo](https://bitbucket.org/rosenstroem/dda_prepost/src/master/)
